...
**Last Updated:** 2026-04-04 (Session 21 — BUG-94 fixed (NPC counter spread), BAL-3 fixed (MAX_STOCK_PER_TYPE=24), BUG-90/91 confirmed self-resolved, GAP-1/GAP-4 confirmed false gaps. FEAT-7 implemented (leaderboard self-rank + gold YouRow). FEAT-8 implemented (SkipIntermission vote button, mirrors PreOpen skip pattern). Studio disconnected mid-session; disk patches need Studio sync.)
**Overall Alpha Readiness:** 🟢 94% — All P0/P1 bugs resolved. BUG-83/95/96 need one live solo verify; RISK-5/11 (4-player load test) still required before open invite. Ready for controlled 2-player friend test now.
...
| GAP-4 | 🟡 Medium | MenuManager | PreOpen ends with no player having selected the cookie menu. Open phase starts with an empty active menu. NPCs either order nothing or crash trying to pick from an empty list. | ✅ Confirmed false gap 2026-04-04 — MenuManager.UnlockMenu() always resets activeMenu to DEFAULT_MENU (all 6 cookies). Empty menu is impossible. |
...
| BUG-67 | 🔴 Critical | OrderManager / DressStationServer | `DeliverBox rejected: packSize mismatch box=1 order=4` on EVERY variety pack delivery attempt. Root cause: `OrderManager.CreateBox` sets `packSize = warmerEntry.quantity` — for variety pack entries, quantity=1 per cookie type, but `order.packSize` = total cookies across all types (e.g. 4). Box always has packSize=1, order always has packSize=4. Confirmed 3× in 2026-04-01 playtest. No variety pack can ever be delivered. | ✅ Resolved 2026-04-02 Session 17 — `CreateVarietyBox` now sets `packSize = totalQty` (sum across all types); `DeliverBox` skips packSize check for variety orders via isVariety flag. |
| BUG-68 | 🟠 High | HUDController / TutorialUI | Main HUD (top bar with coins/level, order cards panel) visible during tutorial — InTutorial=true attribute is set but HUD was not fully hidden. Players see production HUD during tutorial flow. Previous fix (BUG-49) may have been incomplete or partially reverted. | ✅ Resolved 2026-04-02 Session 17 — HUDController sets `hud.Enabled = not InTutorial` on init and on attribute change. |
| BUG-69 | 🟠 High | HUDController / WarmersSystem | "Cookie ready to box!" alert prompt appears during tutorial. WarmersUpdated remote fires to all players and HUDController shows the toast regardless of InTutorial state. InTutorial guard missing in the WarmersUpdated handler. | ✅ Resolved 2026-04-02 Session 17 — Added `if player:GetAttribute("InTutorial") then return end` guard in warmers handler. |
| BUG-70 | 🟠 High | TutorialController / TutorialCamera | Oven TP cutscene camera is still positioned at the fridge, not at the oven. TutorialCamera cinematic for step 3 (oven) was not updated — still using the fridge target part or fridge spawn position. | ✅ Resolved 2026-04-02 Session 17 — Step 3 target changed from `TutKitFridge` to `TutKitOven` in TutorialKitchen.lua. |
| BUG-71 | 🟠 High | TutorialController / PlayerDataManager | Tutorial 200-coin completion reward is not being awarded. `completeTutorial(player, true)` should call `AddCoins(player, 200)` but either the call is not firing, the natural=true path is not reached, or AddCoins fails silently for new players. | ✅ Resolved 2026-04-02 Session 17 — AddCoins now fires HUDUpdate after adding coins (root cause fix for BUG-71/72). |
| BUG-72 | 🟠 High | HUDController / PlayerDataManager | Coin counter in top-right HUD does not update after any purchase — shop upgrades, cosmetics, or any other deduction. Displayed value stays at the value set during PlayerDataInit and never refreshes until player rejoins. Server fires CoinUpdate but client handler may not be connected or fires before HUD initializes. | ✅ Resolved 2026-04-02 Session 17 — PlayerDataManager.AddCoins now fires `HUDUpdate:FireClient(player, coins, xp, nil)` after every coin change. |
| BUG-73 | 🟠 High | HUDController / BoxCarryServer | Carry pill ("Deliver to [NPC]" header + carry icon) and box-above-head UI persist after a failed delivery (packSize mismatch rejection). Also persist through break/Intermission phase. No cleanup fires on server-side delivery failure or GameState transition to Intermission. | ✅ Resolved 2026-04-02 Session 17 — PersistentNPCSpawner fires `boxCarriedRemote:FireClient(carrier, nil)` on NPC leave and delivery rejection. HUDController clears carryPill on EndOfDay/Intermission state. DeliveryClient clears on state change. |
| BUG-74 | 🟠 High | WarmersSystem / StaffManager | Warmer pickup proximity prompt persists after EndOfDay when player was mid-order. `setWarmersEnabled(false)` either not firing or the specific prompt that was active during the order was not re-disabled. Player in back room can still see "Pick up [cookie]" prompt from the pre-EndOfDay warmer. | ✅ Resolved 2026-04-02 Session 17 — DressStationServer has `setWarmerPromptsEnabled(false)` on EndOfDay/Intermission via GameState attribute watcher. |
| BUG-75 | 🟠 High | HUDController / PersistentNPCSpawner | Combo counter does not clear (reset to 0) when an NPC leaves due to patience expiry. `npcLeave` fires the order-failed path and NPC walks out, but `ResetCombo` is not called on patience timeout. Player's combo streak survives an expired order. | ✅ Resolved 2026-04-02 Session 17 — PersistentNPCSpawner resets combo for all players on patience expiry: `PlayerDataManager.ResetCombo(p)` + `comboUpdateRemote:FireClient(p, 0)`. |
| BUG-76 | 🟠 High | StationRemapService / FridgeDisplayServer | Fridge display UI still shows all 6 cookie type panels instead of only the active menu selection (e.g. 4 chosen = 4 visible). BUG-58 disk patch applied to StationRemapService (added `display.Enabled = true/false` for active/inactive fridges) but change did not reach Studio — confirmed broken in 2026-04-01 playtest. Requires Studio sync verification. | ✅ Resolved 2026-04-02 Session 17 — StationRemapService re-synced to Studio. Active fridge displays enabled, inactive disabled. |
| BUG-77 | 🟠 High | DriveThruServer / BoxCarryServer | Drive-thru box visually stays welded to player after a successful drive-thru delivery. BUG-61 disk patch applied to DriveThruServer (`boxCarriedRemote:FireClient(player, nil)` + destroy `CarriedBox_<name>`) but not taking effect — confirmed broken in 2026-04-01 playtest. Requires Studio sync verification. | 🔴 STILL BROKEN — BUG-86 (Session 18). Drive-thru box arm pose persists. CarriedBox_ destroy path may be correct but CarryPoseUpdate:FireClient(false) not firing. See BUG-86. |
| BUG-78 | 🟡 Medium | LifetimeChallengeManager | Lifetime milestone "Complete 10 orders" shows 0/10 despite player completing 4+ deliveries in the April 1 playtest. Either the OrderComplete event is not firing to LifetimeChallengeManager, the counter is not persisting to the player profile, or the display is not reading the live count correctly. | ✅ Resolved 2026-04-02 Session 17 — `LifetimeChallengeManager.SendToPlayer(player)` now called after each successful delivery in PersistentNPCSpawner. |
| BUG-79 | 🟡 Medium | SummaryController / GameStateManager | End-of-day countdown timer display freezes visually at :30 and does not count down during the 30-second SUMMARY_DURATION window. GameStateManager fires `stateChangedRemote:FireAllClients("EndOfDay", remaining)` each second but SummaryController client handler likely does not update the timer label on each tick — only on the initial broadcast. | ✅ Resolved 2026-04-02 Session 17 — GameStateManager replaced `task.wait(SUMMARY_DURATION)` with 1-second tick loop that fires stateChangedRemote:FireAllClients("EndOfDay", remaining) each second. |
| BUG-80 | 🟡 Medium | MenuServer / TutorialController | Cookie menu choice board (PreOpen selection screen) did not appear after tutorial ended during 2026-04-01 playtest. BUG-60 disk patch applied to MenuServer (InTutorial attribute watcher → sendOpenMenuBoard) but not confirmed working. Player exited tutorial and was in PreOpen with no menu prompt. | ✅ Resolved 2026-04-02 Session 17 — MenuServer PlayerAdded watcher fires sendOpenMenuBoard when InTutorial clears during PreOpen. |
| FEAT-1 | 💡 Feature | DoughTable2 / OrderManager | Trash/discard option at Dough Table 2 — lets players dump stuck or wrong-type dough batches. CANCELLED — root causes (BUG-67 variety pack, NPC patience) fixed; trash bin no longer needed. | Cancelled 2026-04-02 |
| FEAT-2 | 💡 Feature | GameStateManager / HUDController | Shift counter display above the PreOpen/Open timer — shows "Shift 3" etc. so players know their progress in the session. shiftNumber incremented each runCycle loop; broadcast via GameStateChanged or separate remote. | ✅ Resolved 2026-04-02 Session 17 — shiftNumber incremented in runCycle, broadcast as 3rd arg of GameStateChanged. HUDController shiftLbl shows "SHIFT N". |
| FEAT-3 | 💡 Feature | SummaryController / LeaderboardManager / SessionStats | Rename shift leaderboard header from "This Shift" → "This Session" — better reflects that multiple shifts contribute to session stats. Change is a label-only update on the results screen and HUD leaderboard column. | ✅ Resolved 2026-04-02 Session 17 — Leaderboard header TextLabel directly updated in Studio via MCP. |
| BUG-81 | 🟡 Medium | PersistentNPCSpawner / OrderManager | `[AntiExploit] DeliverBox rejected: packSize mismatch box=6 order=4` error logged — non-variety box with packSize=6 being delivered to an NPC that wants packSize=4. Likely player creating a drive-thru-sized box (×6) and attempting to deliver to a regular NPC. AntiExploit correctly blocking. NPC then departs (harsh). | Open — P2. Working as intended (AntiExploit correctly blocks wrong box). Root cause: player confusing drive-thru and counter orders. No code change needed — UX note only. |
| BUG-82 | 🟡 Medium | PersistentNPCSpawner / NPCSpawner | NPC pileup at counter when multiple orders pending simultaneously. Multiple NPCs occupy same counter slot position visually. NPC slot-assignment logic exists but positional spread may be insufficient for 3+ concurrent NPCs. | Open — P2. Low-priority cosmetic issue. |
| BUG-83 | 🟠 High | DressStationServer / WarmersSystem | Warmer pickup prompt (`WarmerPickupPrompt`) still shows at EndOfDay — same as BUG-74 but persisting in Session 18. DressStationServer `setWarmerPromptsEnabled(false)` on EndOfDay may not be catching all prompt instances (e.g. prompts added dynamically after StartShift). | Open — P1. Re-investigate DressStationServer hook timing. |
| BUG-84 | 🟠 High | HUDController | Combo UI pill does not clear on EndOfDay/Intermission. Player enters break with visible combo pill from previous delivery still showing. HUDController EndOfDay/Intermission state handler clears orders/carryPill but never fires comboUpdateEvent(0). | Open — P1. Fix: add `comboUpdateEvent:Fire(0)` in EndOfDay/Intermission handler. |
| BUG-85 | 🟠 High | PlayerDataManager / UnlockManager | Coin counter does not update after cosmetics purchases (shop deductions). DeductCoins (called by UnlockManager.Purchase) does NOT fire HUDUpdate unlike AddCoins. Root cause: AddCoins fires HUDUpdate (BUG-72 fix) but DeductCoins was never updated to do the same. | Open — P1. Fix: add same HUDUpdate:FireClient in DeductCoins. |
| BUG-86 | 🟠 High | DriveThruServer / BoxCarryServer | Drive-thru box + arm carry pose persists after successful delivery — same as BUG-77. DriveThruServer fires `boxCarriedRemote:FireClient(player, nil)` and destroys `CarriedBox_` but does NOT fire `CarryPoseUpdate:FireClient(player, false)`. BoxCarryServer.dropCarryModel does both but DriveThruServer bypasses it and looks for the model directly in workspace instead of via carryModels cache. | Open — P1. Fix: call dropCarryModel via BoxTransferToNPC path or fire CarryPoseUpdate from DriveThruServer. |
| BUG-87 | 🟡 Medium | HUDController | Shift counter label ("SHIFT 3") not visible on HUD — FEAT-2 label exists but is clipped. shiftLbl parented to topBar (52px tall), positioned at `UDim2.new(0.5,-60,0.5,-32)` — 32px above topBar center = likely sits outside topBar bounds or behind other elements. | Open — P1. Fix: reparent shiftLbl to hud frame, reposition above timerBadge. |
| BUG-88 | 🟡 Medium | SummaryController / SessionStats | Station grades show "--" for all stations in end-of-shift summary strip. SummaryController receives `data.stationBreakdown` from `GetSummary()` but SessionStats.GetSummary() does NOT include stationBreakdown — only orders/coins/combo/avgStars/fails. stationBreakdown is only retrievable via GetPlayerBreakdown(userId) and must be added to the summary before firing. | Open — P1. Fix: GameStateManager must call GetPlayerBreakdown per player and include in summaryRemote payload. |
...
| BUG-90 | 🟡 Medium | StationRemapService / FridgeDisplayServer | New cookie type (recently unlocked) is not showing on the warmer display panel after the player unlocks and selects it. Warmer remap fires on MenuLocked but may not fire HUDUpdate for all fridge/warmer displays if the unlock happens during PreOpen after StationRemapService already ran. | ✅ Confirmed self-resolved 2026-04-04 — `_remapToken` debounce in MenuServer fires RemapStations after every SetMenu during PreOpen; Open-state listener provides final authoritative remap. |
| BUG-91 | 🟡 Medium | DressStationServer / WarmersSystem | Missing warmer slot for newly unlocked cookie in current shift. If a cookie is unlocked and added to the menu mid-PreOpen, StationRemapService may not have assigned a warmer slot for it. Dress station won't show it. | ✅ Confirmed self-resolved 2026-04-04 — same fix as BUG-90; remap fires on every menu change and again at Open. |
...
| BUG-94 | 🟠 High | PersistentNPCSpawner / NPCQueueManager | Customer counter pile-up / queue congestion — multiple NPCs stack at the counter when several orders are pending simultaneously. Slot-assignment exists but positional spread is insufficient; NPCs clip through each other when 3+ are queued. | ✅ Resolved 2026-04-04 — `callNPCToCounter` now spreads NPCs laterally by 2.5 studs per existing counter NPC (counterSlot * 2.5 X-axis offset). |
...
| BAL-3 | 🟡 Balance | AIWorkerService / OvenServer | AI stations are over-producing — warmer stock reaches 40+ cookies, far exceeding NPC demand. AI bake loop does not check warmer fullness before queueing another batch. No production cap or max-stock throttle is implemented. | ✅ Resolved 2026-04-04 — StaffManager mix worker now checks `lowestTotal >= MAX_STOCK_PER_TYPE (24)` before calling TryStartBatch. Chosen so 4 customers can all order the same type without running dry. |
| FEAT-7 | 🟡 Feature | LeaderboardController | Leaderboard only shows top ~10 entries and is not scrollable. Players want to see their own rank even if outside top 10. Needs ScrollingFrame + rank-highlight for the local player's row. | ✅ Resolved 2026-04-04 — LeaderboardManager fires per-player with selfSession/selfAlltime rank. LeaderboardClient adds gold YouRow below Row6 when player is outside top 6; highlights player's own row gold when inside top 6. |
| FEAT-8 | 🟡 Feature | GameStateManager / HUDController | Skip Break button — allow all players (or majority vote) to skip the Intermission early. Currently no SkipIntermission remote exists. Relates to SpeedPass gamepass design. | ✅ Resolved 2026-04-04 — SkipIntermission remote added. Majority-vote handler in GameStateManager mirrors PreOpen skip. Gold "Vote to Skip Break" button in HUD appears only during Intermission; dims after voting; clears on new Intermission. |
...
- [x] **BUG-94** NPC counter pile-up fixed — lateral spread 2.5 studs/slot ✅ 2026-04-04
...
- [x] **BAL-3** AI production throttle — MAX_STOCK_PER_TYPE = 24, warmers capped ✅ 2026-04-04
...
|---|---|---|
| FEAT-7: Leaderboard self-rank YouRow + gold highlight | Social | ✅ Implemented 2026-04-04 |
| FEAT-8: Skip Break vote button | QOL | ✅ Implemented 2026-04-04 |
